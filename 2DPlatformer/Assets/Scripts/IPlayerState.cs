...
public class JumpingState : IPlayerState
{
    public void EnterState(PlayerMovement player)
    {
        if (player.CanJump())
        {
            Debug.Log(player.MyRigid.velocity.x);
            player.JumpTime++;
            player.MyRigid.velocity = new Vector2(player.MyRigid.velocity.x, player.MyRigid.velocity.y + 10);
        }
    }
    ...
    public void UpdateState(PlayerMovement player)
    {
        ...
        if (player.JumpTime > 0 && player.MyRigid.velocity.y == 0)
        {
            Debug.Log("점프 후 아이들...");
            player.ChangeState(new IdleState());
        }
